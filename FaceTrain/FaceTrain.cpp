...
void backprop_face(IMAGELIST *trainlist, IMAGELIST *test1list, IMAGELIST *test2list, int epochs, int savedelta, char* netname,
				   int list_errors,int hidden_num);
pair<double,double> performance_on_imagelist(BPNN *net,IMAGELIST *il,int list_errors);
void load_input_with_image(IMAGE *img,BPNN *net);
void load_target(IMAGE *img,BPNN *net);
void load_input_with_image(IMAGE *img,BPNN *net);
void load_target(IMAGE *img,BPNN *net);
void bpnn_save_performance(int hidden_num, pair<double, double> perfTrain, pair<double, double> perfTest1, pair<double, double> perfTest2);
			  ...
			  int list_errors,int hidden_num)
{
	...
	if ((net = bpnn_read(netname)) == NULL) {
		if (train_n > 0) {
			...
			*/
			net = bpnn_create(imgsize, /*25: original number 4*/ hidden_num, 4);
		} else {
	...
	for (epoch = 1; epoch <= epochs; epoch++) {
		...
		printf("[%d]: %d ", hidden_num,epoch);  fflush(stdout);
		...
		for (i = 0; i < train_n; i++) {
			...
			bpnn_train(net, 0.9, 0.3, &out_err, &hid_err);
		...
		pair<double,double> perfTrain = performance_on_imagelist(net, trainlist, 0);
		pair<double,double> perfTest1 = performance_on_imagelist(net, test1list, 0);
		pair<double,double> perfTest2 = performance_on_imagelist(net, test2list, 0);
		printf("\n");  fflush(stdout);

		if (epoch == epochs) {
			bpnn_save_performance(hidden_num, perfTrain, perfTest1, perfTest2);
		}
...
void bpnn_save_performance(int hidden_num,pair<double,double> perfTrain,pair<double,double> perfTest1,pair<double,double> perfTest2) {
	FILE* fperf = fopen("performence.txt", "a");
	fprintf(fperf,"%d %g %g %g %g %g %g\n", hidden_num, perfTrain.first, perfTrain.second, perfTest1.first, perfTest1.second, perfTest2.first, perfTest2.second);
	fclose(fperf);
}
...
int evaluate_performance(BPNN *net,double *err)
{
	*err = 0;
	for (int i = 1; i <= 4; i++) {
		*err += (net->target[i] - net->output_units[i])*(net->target[i] - net->output_units[i]);
	}
	*err *= 0.5;

	int idx_max_target = 1;
	double max_target = net->target[1];
	for (int i = 2; i <= 4; i++) {
		if (max_target < net->target[i]) {
			max_target = net->target[i];
			idx_max_target = i;
		}
	}

	int idx_max_output = 1;
	double max_output = net->output_units[1];
	for (int i = 2; i <= 4; i++) {
		if (max_output < net->output_units[i]) {
			max_output = net->output_units[i];
			idx_max_output = i;
		}
	}

	return idx_max_output == idx_max_target;
...
std::pair<double,double> performance_on_imagelist(BPNN *net,IMAGELIST *il,int list_errors)
{
	double err, val;
	int i, n, j, correct;
	...
	if (n > 0) {
		...
		return std::make_pair(100.0 * correct / n, err);
	} else {
		if (!list_errors)
			printf("0.0 0.0 ");
