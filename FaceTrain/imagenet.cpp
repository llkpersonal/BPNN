...
void load_target(IMAGE *img,BPNN *net)
{
  ...
  if (!strcmp(expression, "angry")) {
	  ...
	  net->target[2] = net->target[3] = net->target[4] = TARGET_LOW;
  }
  else if (!strcmp(expression, "happy")) {
	  net->target[2] = TARGET_HIGH;
	  net->target[1] = net->target[3] = net->target[4] = TARGET_LOW;
  }
  else if (!strcmp(expression, "neutral")) {
	  net->target[3] = TARGET_HIGH;
	  net->target[1] = net->target[2] = net->target[4] = TARGET_LOW;
  }
  else {
	  net->target[4] = TARGET_HIGH;
	  net->target[1] = net->target[2] = net->target[3] = TARGET_LOW;
  }
